...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	speed := meanSpeed(action, duration)
	heightWalk := float64(action) * lenStep / cmInM
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speed*kmhInMsec, 2)/heightWalk)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
